    ...
    <div class="x_panel">
      <div class="x_title">
        ...
        <ul class="nav navbar-right panel_toolbox">
          <li>
            <button class="btn btn-sm btn-primary" type="button" @click="$router.push({ name: 'app.menu.form' })">
              <i class="fa fa-plus" />
              Tambah
            </button>
          </li>
                  ...
                  <table class="table table-striped jambo_table bulk_action table-bordered">
                    ...
                    <tbody>
                      <tr v-for="(user, i) in data" :key="i">
                        ...
                        <td>
                          {{ $filters.currency(user.harga) }}
                        </td>
                        <td>
                          {{ user.kategori_menu }}
                        </td>
                        <td>
                          <span class="badge bg-primary" v-if="user.status =='Aktif'">{{ user.status }}</span>
                          <span class="badge bg-danger" v-else>{{ user.status }}</span>
                        </td>
...
export default {
  ...
  methods: {
    ...
    _remove(user, newStatus) {
      ...
      this.$axios
        .delete(`/app/menu/${user.id_menu}`, params)
        .then((res) => {
          this.$root.notif(res.message);
          this.loadData();
        })
